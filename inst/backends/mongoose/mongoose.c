...
#define MG_DISABLE_CGI // CGI is not supported
...
#include <io.h>
#else // _WIN32_WCE
#define MG_DISABLE_CGI // WinCE has no pipes
...
//        cry(conn, "%s: cannot open %s: %s", __func__, fname, strerror(errno));
        LOGGER(0, ("%s: cannot open %s", __func__, fname));
      }
      break;
    }
  }
...
static void handle_directory_request(struct mg_connection *conn,
  ...
  free(data.entries);
  conn->num_bytes_sent += mg_chunked_printf(conn, "{\"key\":\"\",\"mod\":\"\",\"size\":\"\"}]");
...
// XXX ADD SUPPORT HERE FOR CHUNKED ENCODING UPLOADS TODO
// XXX (content-length is required below)
static int forward_body_data(struct mg_connection *conn, FILE *fp,
  ...
  if (conn->content_len == -1) {
    ...
    LOGGER(2, ("PUT %lld bytes", (long long)conn->content_len));
...
mg_post_upload(struct mg_connection *conn, char *filename, int append, int lock)
{
  ...
  LOGGER(1, ("post_upload expecting %s bytes\n", content_length));
  if(clen < 1) return -2;
  fp = NULL;
  ...
  while ((n = mg_read(conn, buf, MG_BUF_LEN)) > 0 && len < clen)
  {
    ...
    {
      LOGGER(0, ("post_upload short fwrite %d\n", ferror(fp)));
      len = -1;
      break;
    }
  }
  if(n < 0)
  {
    LOGGER(0, ("post_upload connection read error\n"));
    len = -1;
  }
  funlockfile(fp);
  fclose(fp);
...
static void redirect_to(struct mg_connection *conn)
{
  struct mg_request_info *ri = &conn->request_info;
  LOGGER(1, ("redirecting request to %s", conn->ctx->config[REDIRECT_TO]));
  if(ri->query_string != NULL)
  {
    mg_printf(conn, "HTTP/1.1 307 Found\r\nLocation: %s%s?%s\r\n\r\n", conn->ctx->config[REDIRECT_TO], ri->uri, ri->query_string);
  } else
  {
    mg_printf(conn, "HTTP/1.1 307 Found\r\nLocation: %s%s\r\n\r\n", conn->ctx->config[REDIRECT_TO], ri->uri);
  }
}
...
static void handle_request(struct mg_connection *conn)
{
  ...
  hdr = mg_get_header(conn, "Authorization");
  if(hdr != NULL)
  {
    LOGGER(1, ("%s %s (auth)", ri->request_method, ri->uri));
  } else LOGGER(1, ("%s %s", ri->request_method, ri->uri));
...
//    send_authorization_request(conn);
    ...
    mkcol(conn, path);
      ...
      if(!mg_stat(conn, path, &de.file)) {
          if(conn->ctx->config[REDIRECT_TO] != NULL)
            {
              redirect_to(conn);
            }
          else
          ...
          else send_http_error(conn, 404, "Not Found", "%s", "File not found");
...
static int load_dll(struct mg_context *ctx, const char *dll_name,
  ...
  if ((dll_handle = dlopen(dll_name, RTLD_LAZY)) == NULL) {
    LOGGER(0, ("%s: cannot load %s", __func__, dll_name));
    return 0;
  }
...
//      ctx->callbacks.init_ssl == NULL) {
  ...
  if (!load_dll(ctx, SSL_LIB, ssl_sw) ||
    ...
    LOGGER(0, ("set_ssl_option:  failed to load dll"));
    return 0;
  }
#endif // NO_SSL_DL
  ...
  if ((ctx->ssl_ctx = SSL_CTX_new(SSLv23_server_method())) == NULL) {
    LOGGER(0, ("set_ssl_option: SSL_CTX_new (server) error: %s", ssl_error()));
    return 0;
  }
  ...
  if ((ctx->callbacks.init_ssl == NULL ||
    ...
    LOGGER(0, ("set_ssl_option: %s: cannot open %s: %s", __func__, pem, ssl_error()));
    return 0;
  }
  ...
  if ((ssl_mutexes = (pthread_mutex_t *) malloc((size_t)size)) == NULL) {
    LOGGER(0, ("set_ssl_option: %s: cannot allocate mutexes: %s", __func__, ssl_error()));
    return 0;
  }
...
static int set_gpass_option(struct mg_context *ctx) {
  ...
  if (path != NULL && !mg_stat(fc(ctx), path, &file)) {
    LOGGER(0, ("can't open global password file"));
    return 0;
  }
  return 1;
}
...
//    }
    ...
    LOGGER(2, ("set option %s to %s", name, value));
...
#endif
      !set_acl_option(ctx)) {
    LOGGER(0, ("mg_start: failed to set option."));
    free_context(ctx);
    return NULL;
  }
...
main (int argc, char **argv)
{
  ...
  options[7] = "realm";
  options[8] = "global_auth_file";
  options[9] = NULL;
  options[10] = "redirect_to";
  options[11] = NULL;
  options[12] = NULL;
  ...
  for (i = 1; i < argc; i++)
    {
      if (strcmp (argv[i], "-d") == 0 && i + 1 < argc)
        ...
        {
          LOGGER_LEVEL = atoi(argv[++i]);
        }
      else
        {
          fprintf(stderr, "Unknown option: [%s]\n", argv[i]);
          fprintf(stderr, "usage:\n\n");
          fprintf(stderr, " ./mongoose [-d root_path] [-p port]\n    [-s ssl_cert_file] [-a auth_domain]\n    [-P full_path_to_global_auth_file]\n    [-l log_level(0-2)] [-f forward_to_host]\n");
          exit (1);
        }
    }
    if(options[5] != NULL)
      {
        LOGGER(1, ("enabling ssl on port %s", options[1]));
        snprintf(prt, 128, "%ss", options[1]);
        options[1] = prt;
      }
  ...
  if (!ctx)
    {
      LOGGER(0, ("failed to start web service"));
      return -1;
    }

  LOGGER(1, ("started web server"));
  for (;;)
    sleep (100);
