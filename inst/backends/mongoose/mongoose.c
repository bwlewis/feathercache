/* Adapted from the mongoose server, GPL version 2 software license
...
static void mg_handle_ssi_request(struct mg_connection *nc, const char *path,
  ...
  if ((fp = fopen(path, "rb")) == NULL) {
    LOG(LL_DEBUG, ("mg_handle_ssi_request 404 error"));
...
static void mg_http_send_file2(struct mg_connection *nc, const char *path,
  ...
  if ((pd->file.fp = fopen(path, "rb")) == NULL) {
    ...
    switch (errno) {
      ...
      case ENOENT:
        code = 404;
        LOG(LL_DEBUG, ("mg_http_send_file2 404 error"));
        break;
      default:
        code = 500;
    };
...
static int mg_http_check_digest_auth(struct http_message *hm,
  ...
  char user[50], response[40], uri[200], qop[20], nc[20], nonce[30];
  char expected_response[33], cnonce[128];

  hdr = mg_get_http_header(hm, "Authorization");
  mg_http_parse_header(hdr, "cnonce", cnonce, sizeof(cnonce));

  /* Parse "Authorization:" header, fail fast on parse error */
  if (hm == NULL || fp == NULL || hdr == NULL ||
...
static void mg_send_directory_listing(struct mg_connection *nc, const char *dir,
  ...
  mg_printf_http_chunk(nc, "{\"key\":\"\",\"mod\":\"\",\"size\":\"\"}]");
...
static void mg_handle_delete(struct mg_connection *nc,
  ...
  cs_stat_t st;
  if (mg_stat(path, &st) != 0) {
    LOG(LL_DEBUG, ("mg_handle_delete 404 error"));
    mg_http_send_error(nc, 404, NULL);
...
MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path,
  ...
  if (is_dav && opts->dav_document_root == NULL) {
    ...
    LOG(LL_DEBUG, ("mg_send_http_file 404 error"));
    if(opts->redirect != NULL) mg_redirect(nc, hm, opts);
    else mg_http_send_error(nc, 404, NULL);
#ifndef MG_DISABLE_DAV
...
void mg_serve_http(struct mg_connection *nc, struct http_message *hm,
  ...
  if (mg_uri_to_local_path(hm, &opts, &path, &path_info) == 0) {
    LOG(LL_DEBUG, ("mg_uri_to_local_path 404 error"));
    mg_http_send_error(nc, 404, NULL);
    return;
  }
...
static void mg_redirect(struct mg_connection *nc,
                        struct http_message *hm,
                        struct mg_serve_http_opts *opts)
{
  LOG(LL_INFO, ("redirect: %s%.*s", opts->redirect, (int)hm->uri.len, hm->uri.p));
  mg_send_response_line(nc, 307, opts->extra_headers);
  if(hm->query_string.len > 0)
  {
    mg_printf(nc, "%s: %s%.*s?%.*s\r\n\r\n", "Location", opts->redirect,
      (int)hm->uri.len, hm->uri.p, (int)hm->query_string.len, hm->query_string.p);
  } else
  {
    mg_printf(nc, "%s: %s%.*s\r\n\r\n", "Location", opts->redirect, (int)hm->uri.len, hm->uri.p);
  }
  nc->flags |= MG_F_SEND_AND_CLOSE;
}
      ...
      if (strcmp (argv[i], "-d") == 0 && i + 1 < argc)
        {
          ...
          s_http_server_opts.dav_document_root = argv[i];
        }
        ...
        {
          s_http_server_opts.redirect = argv[++i];
        }
      else if (strcmp (argv[i], "-P") == 0 && i + 1 < argc)
        {
          if(s_http_server_opts.per_directory_auth_file != NULL)
          {
            fprintf (stderr, "Only one of the -P and -A options may be used at a time.\n");
            exit (1);
          }
        ...
        {
          if(s_http_server_opts.global_auth_file != NULL)
          {
            fprintf (stderr, "Only one of the -P and -A options may be used at a time.\n");
            exit (1);
          }
      ...
      else
        {
          ...
          fprintf (stderr,
                   " ./mongoose [-d root_path] [-p port] [-s ssl_cert_file] [-a auth_domain]\n            [-P global_auth_file] [-A per_directory_auth_file]            [-l log_level(0-5)] [-f forward_to_host]\n");
          exit (1);
        }
    }
